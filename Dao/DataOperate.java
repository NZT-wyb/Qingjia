...
public class DataOperate {
    ...
    public static StringBuilder getStuno() {//不使用String，因为需要大量拼接字符串
        StringBuilder xh=new StringBuilder("20194");//学号前7位相同
        StringBuilder xh1=new StringBuilder(String.valueOf(getNum(1,10000)));//随机取后5位 2019401284
        if(xh1.length()==1) {
            ...
            xh1=xh1.insert(0, "0");//如果4位数，前面增加1个0
            xh=xh.append(xh1);
        }else {
            xh=xh.append(xh1);
        }
    ...
    public static void addSJ() throws SQLException {//增加成绩
        ...
        String sql="insert into qingjia(name,number,class,tel,date1,date2,reason,status) values(?,?,?,?,?,?,?,?)";//使用占位符定义插入语句
        conn=Daotool.getconnection();//获取数据库接
            ...
            for (int i = 1; i <= 2; ) {
                ...
                if (!alist.contains(xh)) {//判断学号是否唯一
                    ...
                    int date2=getNum(1, 30);//随机获取日
                    int date1=getNum(1,12);//随机获取月
                    pstmt.setString(1, xm);//定义第1个占位符的内容
                    pstmt.setString(2, xh);
                    pstmt.setString(3, "软件一班");
                    pstmt.setString(4, "1234");
                    pstmt.setString(5, "2020-"+date1+"-"+date2);
                    pstmt.setString(6, "2020-"+date1+"-"+date2);
                    pstmt.setString(7, "生病请假");//定义第2个占位符的内容
                    pstmt.setString(8, "待审核");//定义第3个占位符的内容
